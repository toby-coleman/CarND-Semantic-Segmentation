...
def load_vgg(sess, vgg_path):
    ...
    w_keep_prob = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    ...
    return w_input, w_keep_prob, w_layer3, w_layer4, w_layer7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    decon_2 = tf.layers.conv2d_transpose(skip_1,
                                         num_classes,
                                         kernel_size=4,
                                         strides=2,
                                         padding='same',
    ...
    decon_3 = tf.layers.conv2d_transpose(skip_2,
                                         num_classes,
                                         kernel_size=16,
                                         strides=8,
                                         padding='same',
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # Reshape to 2d tensor (row=pixels, cols=classes)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        print('Setting up tensorflow variables')
        input_image, keep_prob, layer3_out, layer4_out, layer7_out = load_vgg(sess, vgg_path)
        layer_out = layers(layer3_out, layer4_out, layer7_out, num_classes)
        lr = tf.placeholder(tf.float32)
        correct_label = tf.placeholder(tf.float32, [None, None, None, num_classes])
        logits, op, loss = optimize(layer_out, correct_label, lr, num_classes)

        # Train NN using the train_nn function
        print('Start training')
        saver = tf.train.Saver()
        if not os.path.exists('./checkpoint_models'):
            os.mkdir('./checkpoint_models')
